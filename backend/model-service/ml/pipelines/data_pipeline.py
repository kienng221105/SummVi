...
class DataPipeline:
    """
    Pipeline xử lý và chuẩn bị dữ liệu training cho model tóm tắt.

    Chức năng:
    - Load raw data từ nhiều format (JSONL, CSV, TXT)
    - Clean và deduplicate data
    - Tính toán features (word count, sentence count, compression ratio)
    - Xuất processed data và feature store
    """
    ...
    def __init__(
        ...
        """Khởi tạo pipeline và tạo các thư mục cần thiết."""
    ...
    def _bootstrap_dataset_if_empty(self) -> None:
        """
        Tạo dataset mẫu nếu thư mục raw còn trống.
        Bootstrap samples giúp pipeline có thể chạy ngay cả khi chưa có data thật.
        """
    ...
    def _build_processed_dataframe(self, records: list[dict]) -> pd.DataFrame:
        """
        Xử lý raw records thành processed dataframe.

        Logic:
        - Clean text và summary
        - Deduplicate theo text content
        - Tự động tạo summary từ 2 câu đầu nếu thiếu
        - Tính toán metrics: char count, word count, sentence count
        """
        rows: list[dict] = []
    ...
    def _build_feature_store(self, processed_df: pd.DataFrame) -> pd.DataFrame:
        """
        Tạo feature store từ processed data.

        Features:
        - compression_ratio_target: tỷ lệ nén mục tiêu (1 - summary_length/text_length)
        - avg_word_length: độ dài trung bình của các từ

        Feature store dùng cho training và evaluation.
        """
    ...
    def run(self) -> dict:
        ...
        Flow:
        1. Bootstrap dataset nếu cần
        2. Load raw records từ tất cả file
        3. Process và clean data
        4. Build feature store
        5. Xuất ra JSONL và CSV
        6. Tạo manifest report

        Returns:
            dict chứa thống kê và đường dẫn output files
        """
