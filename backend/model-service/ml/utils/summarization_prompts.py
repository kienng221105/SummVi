...
def limit_input_words(text: str, max_words: int = MAX_SUMMARY_INPUT_WORDS) -> TruncationResult:
    """
    Giới hạn số từ của văn bản đầu vào để đảm bảo chất lượng tóm tắt.

    Logic:
    - Nếu văn bản <= max_words: giữ nguyên
    - Nếu văn bản > max_words: cắt bớt và đánh dấu truncated=True

    Lý do giới hạn: Model tóm tắt hoạt động tốt nhất với input không quá dài.
    Quá dài sẽ làm giảm chất lượng và tăng latency.
    """
...
def normalize_summary_length(summary_length: str | None) -> str:
    """Chuẩn hóa tham số độ dài tóm tắt về giá trị hợp lệ."""
...
def normalize_output_format(output_format: str | None) -> str:
    """Chuẩn hóa tham số format đầu ra về giá trị hợp lệ."""
...
def generation_max_new_tokens(summary_length: str | None) -> int:
    """
    Xác định số token tối đa cho model generation dựa trên độ dài yêu cầu.
    Short: 128 tokens, Medium: 192 tokens, Long: 280 tokens
    """
...
def build_summary_prompt(text: str, summary_length: str = "medium", output_format: str = "paragraph") -> str:
    """
    Xây dựng prompt đầy đủ cho model tóm tắt ViT5.

    Prompt structure:
    - System instruction: vai trò của model
    - Task description: nhiệm vụ cụ thể
    - Length instruction: yêu cầu về độ dài (short/medium/long)
    - Format instruction: yêu cầu về format (bullet/paragraph)
    - Rules: các quy tắc cần tuân thủ
    - Source text: văn bản cần tóm tắt

    Prompt engineering quan trọng để model tạo output chất lượng cao.
    """
    normalized_length = normalize_summary_length(summary_length)
